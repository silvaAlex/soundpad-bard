...
pub struct SkipBardTrack<'a, O: ObsConnector> {
    pub obs: &'a O,
    pub config: &'a AppConfig,
}

impl<'a, O: ObsConnector> SkipBardTrack<'a, O> {
    pub async fn execute(&self, next_file: &str) -> Result<()> {
        let file_path = self.config.bard.music_dir.join(next_file);
